            ...
            <div class="ibox float-e-margins">
                <div class="ibox-title" style="padding: 14px 20px 10px;">
                    <div class="input-group" style="float: left;width: 40%;">
                        ...
                        <input id="name" type="text" placeholder="请输入商品名称" class="input-sm form-control"
                               style="width: 200px;height: 32px">
                    </div>
                    <!--                    <div class="ibox-tools">-->
                    <!--                        <a><span class="btn btn-primary" onclick="AddOne()">添加商品</span></a>-->
                    <!--                    </div>-->
                ...
                <div class="ibox-content" style="padding: 15px 40px 20px 40px;">
                    <table class="table table-striped table-bordered table-hover dataTables-example nowarp"
                        ...
                        <tbody id="companionship_list"></tbody>
                    </table>
                    <div style="text-align: center;">
                        <div id="companionship_list_page"></div>
                    </div>
                </div>
            </div>
        </div>
        ...
        $.post(url, data, function (res) {
            ...
            for (var i = 0; i < res.length; i++) {
                HtmlStr += '' +
                    ...
                    '    <td>' + (i + 1) + '</td>' +
                    '    <td>' + res[i].name + '</td>' +
                    '    <td>' + res[i].companionship_time + '</td>' +
                    '    <td>' + res[i].address + '</td>' +
                    '    <td>' + res[i].phone + '</td>' +
                    '    <td class="operate">' +
            ...
            $("#companionship_list").html(HtmlStr);
        }, "json");
    }

    function AddOne(id = '') {
        //window.location.href = "<?php //echo base_url() ?>//index.php/Admin/Product/AddOne?id=" + id;
    }
