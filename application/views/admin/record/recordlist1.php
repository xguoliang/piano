            ...
            <div class="ibox float-e-margins">
                <div class="ibox-title" style="padding: 14px 20px 10px;">
                    <div class="input-group" style="float: left;width: 40%;">
                        ...
                        <input id="name" type="text" placeholder="请输入录影棚名称" class="input-sm form-control"
        ...
        $.post(url, data, function (res) {
            ...
            for (var i = 0; i < res.length; i++) {
                HtmlStr += '' +
                    ...
                    '    <td>' + res[i].name + '</td>' +
                    '    <td>' +
                    '       <img src="<?php echo base_url()?>'+res[i].coverimg+'">' +
                    '    </td>' +
                    '    <td>' + res[i].price + '</td>' +
                    '    <td>' + res[i].phone + '</td>' +
                    '    <td>' + res[i].address + '</td>' +
            ...
            $("#record_list").html(HtmlStr);
        }, "json");
    }

    function AddOne(id = '') {
        //window.location.href = "<?php //echo base_url() ?>//index.php/Admin/recordsman/AddOne?id=" + id;
    }
