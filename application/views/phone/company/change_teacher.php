...
<div class="my-edit">
    ...
    <div class="edit-item">
        <span class="edit-fenlei">
            专业</span>
        <input type="text" placeholder="吉他" id="profession" value="<?php echo $detail['profession']; ?>">
    </div>
    <div class="edit-item">
        <span class="edit-fenlei">
            教龄</span>
        <input type="text" placeholder="10年" id="year" value="<?php echo $detail['year']; ?>">
    </div>
    <div class="edit-item">
        <span class="edit-fenlei">
            <em>*</em>课程</span>
    ...
    <div class="edit-item" >
        <ul class="choose-course">
            <?php foreach ($lesson as $data_item): ?>
                <li><i class="iconfont <?php
                    $has = 0;
                    for ($i = 0; $i < count($tlesson); $i++) {
                        if ($tlesson[$i]['lesson_id'] == $data_item['id']) {
                            $has++;
                        }
                    }
                    if ($has == 0) {
                        ?>icon-checkboxround0<?php } else {
                        ?>
                           icon-checkboxround1
                       <?php }
                       ?>" data="<?php echo $data_item['id']; ?>"></i><span><?php echo $data_item['name']; ?></span></li>
                <?php endforeach; ?>
        </ul>
    </div>
