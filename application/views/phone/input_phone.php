...
<div class="login">
    <div class="login-info">
        ...
        <div class="login-form">
            ...
            <button class="login-btn" onclick="submit()">修改</button>
        </div>
    </div>
</div>
<script>
    ...
    var curCount;//当前剩余秒数
    var oldphone = "<?php echo $_SESSION['lerenuser']['phone']?>";
    function sendMessage() {
        ...
        if (validatemobile(phone)) {
            ...
            if(phone == oldphone){
                layer.msg('新手机号不能与原手机号相同!',{time:1500});
                return;
            }
            $.ajax({
                type: "post",
                url: "<?php echo base_url(); ?>index.php/User/Login/sendsms",
                data: {
                    phone: phone,
                },
                dataType: "json",
                success: function (data, status) {
                    if (data.status == 1) {
                        ...
                        layer.msg('输入的手机号已经注册，请直接登录!',{time:1500},function(){
                            window.location.href = "<?php echo base_url()?>User/Login/PLogin";
                        })
                    }
                }
    ...
    function submit() {
        ...
        if(validatemobile(phone)){
            ...
            $.post(url,data,function(res){
                if(res.status == 1){
                    layer.msg('修改成功,请重新登录!',{time:1500},function(){
                        window.location.href = "<?php echo base_url()?>User/Login/PLogin"
                    })
