...
<script>
        ...
        function select_lease() {
            var url = "<?php echo base_url(); ?>User/Student/MyCollectLease";
            $.ajax({
                type: "post",
                url: url,
                data: {
                ...
                success: function (data, textStatus) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        $('#neirong').append('<li id="li_' + data[i].id + '">' +
                                '<div class="rent-img">' +
                                '<a href="<?php echo base_url(); ?>User/Lease/LeaseDetail.html?id=' + data[i].entity_id + '"><img src="<?php echo base_url(); ?>' + data[i].lease.coverimg.split('|')[0] + '" alt=""></a>' +
                                '</div>' +
                                '<div class="rent-txt">' +
                                '<a href="<?php echo base_url(); ?>User/Lease/LeaseDetail.html?id=' + data[i].entity_id + '"><h5 class="rent-name">' + data[i].lease.name + '</h5></a>' +
